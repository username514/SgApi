...
package com.sg.sgapiclientsdk.client;
...
import com.sg.sgapiclientsdk.exception.ErrorCode;
import com.sg.sgapiclientsdk.exception.ErrorResponse;
import com.sg.sgapiclientsdk.exception.SgApiException;
import com.sg.sgapiclientsdk.model.BaseRequest;
import com.sg.sgapiclientsdk.model.entity.User;
import com.sg.sgapiclientsdk.model.enums.UrlToMethodEnum;
import com.sg.sgapiclientsdk.model.params.HoroscopeParams;
import com.sg.sgapiclientsdk.model.params.PublicIpParams;
import com.sg.sgapiclientsdk.model.response.*;
import com.sg.sgapiclientsdk.utils.SignUtils;
import lombok.Data;
...
public class SgApiClient {
    ...
    public SgApiClient(String secretId, String secretKey) {
     ...
     * @return 用户名
     */
    public UserResponse getUsernameByPost(User user) throws SgApiException {
     ...
     * @return 毒鸡汤
     */
    public PoisonousChickenSoupResponse getPoisonousChickenSoup() throws SgApiException {
     ...
     * @return 微博实时热搜榜
     */
    public WeiboHotSearchResponse getWeiboHotSearch() throws SgApiException {
     ...
     * @return 星座运势
     */
    public HoroscopeResponse getHoroscope(HoroscopeParams horoscope) throws SgApiException {
     ...
     * @return 公网ip
     */
    public PublicIpResponse getPublicIp(PublicIpParams publicIp) throws SgApiException {
     ...
     *
     * @return 随机壁纸
     * @throws SgApiException
     */
    public RandomWallpaperResponse getRandomWallpaper() throws SgApiException {
     ...
     * @throws SgApiException
     */
    public LoveTalkResponse getLoveTalk() throws SgApiException {
     ...
     * @throws SgApiException
     */
    public WeatherResponse getWeather() throws SgApiException {
     ...
     * @throws SgApiException
     */
    public Object parseAddressAndCallInterface(BaseRequest baseRequest) throws SgApiException {
        ...
        try {
            ...
            if (path.equals(UrlToMethodEnum.publicIp.getPath())) {
                ...
                if (ipAddress == null || ipAddress.isEmpty()) {
                    throw new SgApiException(ErrorCode.NOT_FOUND_ERROR, "获取公网ip失败");
                }
            ...
            // todo 1.添加新的接口地址判断
        } catch (SgApiException e) {
            throw new SgApiException(e.getCode(), e.getMessage());
        } catch (Exception e){
            throw new SgApiException(ErrorCode.SYSTEM_ERROR, e.getMessage());
        }
        if (ObjUtil.isEmpty(result)) {
            throw new SgApiException(ErrorCode.NOT_FOUND_ERROR, "未找到Url对应资源");
        }
     ...
     * @throws SgApiException
     */
    private Object invokeMethod(String methodName) throws SgApiException {
     ...
     * @throws SgApiException
     */
    private Object invokeMethod(String methodName, Map<String, Object> params, Class<?> paramsType) throws SgApiException {
        try {
            Class<?> clazz = SgApiClient.class;
            if (params == null) {
            ...
            String afdsf = e.getMessage();
            System.out.println("fdsfas");
            throw new SgApiException(ErrorCode.NOT_FOUND_ERROR, "通过url未找到对应方法");
        } catch (Exception e) {
            // 处理异常
            if (e.getCause() instanceof SgApiException) {
                SgApiException gigotApiException = (SgApiException) e.getCause();
                // 处理 SgApiException 异常
                throw new SgApiException(gigotApiException.getCode(),gigotApiException.getMessage());
            }
            throw new SgApiException(ErrorCode.NOT_FOUND_ERROR, e.getMessage());
        }
    }
     ...
     * @param <T>          响应类型
     * @return 响应结果
     * @throws SgApiException 异常
     */
    public <T> T sendRequest(String url, String method, Object params, Class<T> responseType) throws SgApiException {
        HttpRequest request;
        ...
        switch (method) {
            ...
            default:
                throw new SgApiException(ErrorCode.PARAMS_ERROR, "请求方式有误");
        }
        ...
        if (response.getStatus() != 200) {
            ...
            throw new SgApiException(errorResponse.getCode(), errorResponse.getMessage());
        }
        // 获取响应体
     ...
     * @throws SgApiException 业务异常
     */
    public void checkConfig(SgApiClient gigotApiClient) throws SgApiException {
        if (StrUtil.isBlank(gigotApiClient.getSecretId()) && StrUtil.isBlank(gigotApiClient.getSecretKey())) {
            throw new SgApiException(ErrorCode.NO_AUTH_ERROR, "请先配置密钥AccessKey/SecretKey");
        }
    }
